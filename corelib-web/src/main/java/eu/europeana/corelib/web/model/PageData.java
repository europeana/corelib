...
//    }
    ...
    public String getMetaCanonicalUrl() {
        String canonical = StringUtils.defaultIfBlank(metaCanonicalUrl, getCurrentUrl());
        canonical = StringUtils.substringBefore(canonical, "#");
        if (StringUtils.startsWithIgnoreCase(canonical, "http")) {
            return canonical;
        }
        return portalServer + (StringUtils.startsWith(canonical, "/")?StringUtils.substringAfter(canonical, "/"):canonical);
    }
