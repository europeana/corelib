...
import eu.europeana.corelib.solr.server.AbstractServer;
...
import eu.europeana.corelib.solr.server.importer.neo4j.util.AgentFieldInput;
import eu.europeana.corelib.solr.server.importer.neo4j.util.AggregationFieldInput;
import eu.europeana.corelib.solr.server.importer.neo4j.util.ConceptFieldInput;
import eu.europeana.corelib.solr.server.importer.neo4j.util.EuropeanaAggregationFieldInput;
import eu.europeana.corelib.solr.server.importer.neo4j.util.PlaceFieldInput;
import eu.europeana.corelib.solr.server.importer.neo4j.util.ProvidedCHOFieldInput;
import eu.europeana.corelib.solr.server.importer.neo4j.util.ProxyFieldInput;
import eu.europeana.corelib.solr.server.importer.neo4j.util.TimespanFieldInput;
...
public class Neo4jConstructor {
	 ...
	 * @throws IOException
	 */
	public FullBeanImpl constructFullBean(RDF record, Neo4jServer neo4jserver)
		...
		for (ProvidedCHOType pcho : record.getProvidedCHOList()) {
			...
			try {
				providedCHOs.add(new ProvidedCHOFieldInput()
						.createProvidedCHOMongoFields(pcho, neo4jserver));
		...
		for (ProxyType proxytype : record.getProxyList()) {
			...
			proxies.add(new ProxyFieldInput().createProxyMongoFields(
					new ProxyImpl(), proxytype, neo4jserver));
		}
		...
		for (Aggregation aggregation : record.getAggregationList()) {
			...
			aggregations.add(new AggregationFieldInput()
					.createAggregationMongoFields(aggregation, neo4jserver,
							webResourcesMongo));
		...
		if (record.getTimeSpanList() != null) {
			for (TimeSpanType tspan : record.getTimeSpanList()) {
				timespans.add(new TimespanFieldInput()
						.createTimespanMongoField(tspan, neo4jserver));
			}
		}
			...
			for (EuropeanaAggregationType eaggregation : record
				...
				fullBean.setEuropeanaAggregation(new EuropeanaAggregationFieldInput()
						.createAggregationMongoFields(eaggregation,
								neo4jserver, SolrUtils.getPreviewUrl(record)));
			}
		}
