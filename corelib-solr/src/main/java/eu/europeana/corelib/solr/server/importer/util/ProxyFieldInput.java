...
public class ProxyFieldInput {
    ...
    public static SolrInputDocument createProxySolrFields(
        ...
        solrInputDocument.addField(EdmLabel.EDM_TYPE.toString(), SolrUtil.exists(EdmType.class, (providedCHO.getType())).toString());
        ...
        List<eu.europeana.corelib.definitions.jibx.DCTermsType.Choice> dcTermsList = providedCHO.getChoiceList();
        if (dcTermsList != null) {
            for (eu.europeana.corelib.definitions.jibx.DCTermsType.Choice choice : dcTermsList) {
                ...
                if (choice.ifConformsTo()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_CONFORMS_TO.toString(), choice.getConformsTo().getResource());
                }
                if (choice.ifCreated()) {
                ...
                if (choice.ifHasFormat()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_HAS_FORMAT.toString(), choice.getHasFormat().getResource());
                }
                if (choice.ifHasPart()) {
                    solrInputDocument.addField(
                            EdmLabel.DCTERMS_HAS_PART.toString(), choice.getHasPart().getResource());
                }
                if (choice.ifHasVersion()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_HAS_VERSION.toString(), choice.getHasVersion().getResource());
                }
                if (choice.ifIsFormatOf()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_IS_FORMAT_OF.toString(), choice.getIsFormatOf().getResource());
                }
                if (choice.ifIsPartOf()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_IS_PART_OF.toString(), choice.getIsPartOf().getResource());
                }
                if (choice.ifIsReferencedBy()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_IS_REFERENCED_BY.toString(), choice.getIsReferencedBy().getResource());
                }
                if (choice.ifIsReplacedBy()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_IS_REPLACED_BY.toString(), choice.getIsReplacedBy().getResource());
                }
                if (choice.ifIsRequiredBy()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_IS_REQUIRED_BY.toString(), choice.getIsRequiredBy().getResource());
                }
                if (choice.ifIssued()) {
                ...
                if (choice.ifIsVersionOf()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_IS_VERSION_OF.toString(), choice.getIsVersionOf().getResource());
                }
                if (choice.ifMedium()) {
                ...
                if (choice.ifProvenance()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_PROVENANCE.toString(), choice.getProvenance().getResource());
                }
                if (choice.ifReferences()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_REFERENCES.toString(), choice.getReferences().getResource());
                }
                if (choice.ifReplaces()) {
                    solrInputDocument.addField(
                            EdmLabel.DCTERMS_REPLACES.toString(), choice.getReplaces().getResource());
                }
                if (choice.ifRequires()) {
                    solrInputDocument.addField(
                            EdmLabel.DCTERMS_REQUIRES.toString(), choice.getRequires().getResource());
                }
                if (choice.ifSpatial()) {
                ...
                if (choice.ifTableOfContents()) {
                    solrInputDocument.addField(EdmLabel.DCTERMS_TABLE_OF_CONTENTS.toString(), choice.getTableOfContents().getResource());
                }
                if (choice.ifTemporal()) {
                    solrInputDocument.addField(
                            EdmLabel.DCTERMS_TEMPORAL.toString(), choice.getTemporal().getResource());
                }
            }
        }

        // Retrieve the dc namespace fields
        List<eu.europeana.corelib.definitions.jibx.DCType.Choice> dcList = providedCHO.getChoiceList1s();
        if (dcList != null) {
            for (eu.europeana.corelib.definitions.jibx.DCType.Choice choice : dcList) {
                ...
                if (choice.ifDate()) {
                    solrInputDocument.addField(EdmLabel.DC_DATE.toString(), choice.getDate().getResource());
                }
                ...
                if (choice.ifType()) {
                    solrInputDocument.addField(EdmLabel.DC_TYPE.toString(), choice.getType().getResource());
                }
            }
        }
     ...
     * 
     * @param proxy The MongoDB proxy Entity
     * @param aggregation The JiBX Aggregation Entity
     * @param mongoServer The MongoDB Server to save the Entity
