...
public final class TimespanFieldInput {
	...
	public TimespanImpl createTimespanMongoField(TimeSpanType timeSpan,
		...
		TimespanImpl mongoTimespan = (TimespanImpl) neo4jServer.find(timeSpan.getAbout(),new TimespanImpl());
		if (mongoTimespan == null) {
			mongoTimespan = createNewTimespan(timeSpan);

			neo4jServer.save(mongoTimespan);

		} else {
			mongoTimespan = updateTimespan(mongoTimespan, timeSpan, neo4jServer);
		}
		return mongoTimespan;
	}

	private TimespanImpl updateTimespan(TimespanImpl mongoTimespan,
			TimeSpanType timeSpan, Neo4jServer neo4jServer) {
		
		neo4jServer.save(mongoTimespan);

		return  (TimespanImpl) neo4jServer.find(timeSpan.getAbout(),new TimespanImpl());
