...
public final class AgentFieldInput {
	 ...
	 * @param neo4jServer
	 *            - The neo4jServer to save the entity
	...
	public AgentImpl createAgentMongoEntity(AgentType agentType,
			Neo4jServer neo4jServer) throws MalformedURLException, IOException {

		AgentImpl agent = neo4jServer.find(agentType.getAbout(),new AgentImpl());
		...
		if (agent == null) {
			...
			try {
				neo4jServer.save(agent);
			} catch (Exception e) {
				e.printStackTrace();
				AgentImpl agentSec = (AgentImpl) neo4jServer.find(agentType.getAbout(),new AgentImpl());						
				agent = updateMongoAgent(agentSec, agentType, neo4jServer);
				
			}
		} else {
			agent = updateMongoAgent(agent, agentType, neo4jServer);
		}
		return agent;
	}
