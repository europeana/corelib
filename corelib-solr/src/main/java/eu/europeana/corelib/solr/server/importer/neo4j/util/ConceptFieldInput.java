...
public final class ConceptFieldInput {
	...
	public ConceptImpl createConceptMongoFields(Concept concept,
			Neo4jServer neo4jServer, RDF rdf) {

		ConceptImpl conceptMongo = neo4jServer.find(concept.getAbout(), new ConceptImpl());
		if (conceptMongo == null) {
			// If it does not exist
			conceptMongo = createNewConcept(concept);
			try{
				neo4jServer.save(conceptMongo);
			}
			catch(Exception e){
				conceptMongo = updateConcept(conceptMongo, concept, neo4jServer);
			}
		} else {
			conceptMongo = updateConcept(conceptMongo, concept, neo4jServer);
		}
	...
	private ConceptImpl updateConcept(ConceptImpl conceptMongo,
			Concept concept, Neo4jServer neo4jServer) {
		neo4jServer.save(conceptMongo);
		return conceptMongo;
	}
