...
public class Neo4jServerImpl implements Neo4jServer {
	...
	public long getChildrenCount(Node id) {
		...
		PostMethod httpMethod = new PostMethod(serverPath
				+ "transaction/commit");
		try {
	...
	public long getNodeIndex(Node node) {
		...
		try {
			...
			IndexObject obj = new ObjectMapper().readValue(
					method.getResponseBodyAsStream(), IndexObject.class);
	...
	public Hierarchy getInitialStruct(String id) {
		...
		try {
			...
			Hierarchy obj = mapper.readValue(method.getResponseBodyAsStream(),
					Hierarchy.class);
			return obj;
		} catch (HttpException e) {
			e.printStackTrace();
