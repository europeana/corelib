...
public class Node2Neo4jBeanConverter {
    ...
    public static Neo4jStructBean toNeo4jStruct(Hierarchy hierarchy) {
        ...
        List<CustomNode> previous = hierarchy.getPreviousSiblings();
        List<CustomNode> next = hierarchy.getSiblings();
        int i = 0;
    ...
    public static Neo4jBean toNeo4jBean(CustomNode node) {
        if (node != null) {
           ...
           for(String key:keySet){
            ...
            neo4jBean.setIndex(Long.parseLong((String)node.getProperty("index")));
            return neo4jBean;
        }
        return null;
    }
